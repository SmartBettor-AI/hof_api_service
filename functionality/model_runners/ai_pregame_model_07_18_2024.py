        ...
        for strategy_name, strategy_dict in self.model_storage.items():
          ...
          if not filtered_df.empty:
            ...
            if not filtered_df.empty:
               ...
               #Make sure this is mapped properly
               # testing the new prod pull
               ...
               existing_df = pd.read_csv( f'ai_model_output/{self.name}.csv', on_bad_lines='skip')
               print('pulled df')
               print(existing_df)
               print(type(filtered_df))
               existing_df = existing_df[filtered_df.columns]
               result_df = pd.concat([existing_df, filtered_df], ignore_index=True)

            # result_df = map_display_data('wager', result_df)
