...
class pregame_ai():
 ...
 'windcreek_1_points']
    ...
    # Sends date in UTC to the process snapshot 
    ...
    def filter_by_params(self, df):
       ...
       def filter_by_populated_columns(df):
          odds_cols_to_keep = ['draftkings_1_odds', 'betmgm_1_odds', 'fanduel_1_odds', 'williamhill_us_1_odds', 'betrivers_1_odds']
          line_cols_to_keep = ['draftkings_1_points', 'betmgm_1_points', 'fanduel_1_points', 'williamhill_us_1_points', 'betrivers_1_points']

          odds_cols_to_drop = [col for col in df.columns if 'odds' in col and col not in odds_cols_to_keep and col != 'average_market_odds_old' and col != 'highest_bettable_odds']
          line_cols_to_drop = [col for col in df.columns if 'points' in col and col not in line_cols_to_keep and col != 'pointsbetus_1_odds' and col != 'pointsbetus_2_odds']

          df = df.drop(columns=odds_cols_to_drop)
          df = df.drop(columns=line_cols_to_drop)
    ...
    def format_for_nn(self):
       ...
       self.numerical_columns = ['fanduel_1_odds', 'williamhill_us_1_points', 'betrivers_1_points', 'fanduel_1_points', 'draftkings_1_points', 'williamhill_us_1_odds', 'betrivers_1_odds', 'betmgm_1_odds', 'betmgm_1_points', 'draftkings_1_odds', 'average_market_odds_old', 'highest_bettable_odds']
      ...
      if not self.filtered_df.empty:
        ...
        for strategy_name, strategy_dict in self.model_storage.items():
          ...
          if not filtered_df.empty:
            ...
            result_df.to_csv( f'ai_model_output/{self.name}.csv', 
               mode = 'w', 
               index = False
            )
